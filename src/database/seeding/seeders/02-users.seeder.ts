...
export class UsersSeeder implements Seeder {
    ...
    async run(): Promise<void> {
        ...
        const employerRole = await this.roleRepository.findOne({ where: { name: 'EMPLOYER' } });
        ...
        const existingEmployer = await this.userRepository.findOne({ where: { email: employerEmail } });
        const existingStudent = await this.userRepository.findOne({ where: { email: studentEmail } });
