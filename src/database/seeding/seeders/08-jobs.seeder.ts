...
export class JobsSeeder implements Seeder {
    ...
    async run(): Promise<void> {
        ...
        const employerUser = await this.userRepository.findOne({ where: { email: employerEmail } });
        ...
        const jobsArray = (jobsData as unknown as { default?: typeof jobsData }).default ?? jobsData;

        // 2. Loop through the imported JSON array
        for (const jobData of jobsArray as typeof jobsData) {
