"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.JobCategory = void 0;
const typeorm_1 = require("typeorm");
const job_entity_1 = require("../../modules/jobs/job.entity");
const employer_profile_entity_1 = require("../../modules/employer-profiles/employer-profile.entity");
let JobCategory = class JobCategory {
    id;
    name;
    employer;
    isActive;
    createdAt;
    jobs;
};
exports.JobCategory = JobCategory;
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)('uuid'),
    __metadata("design:type", String)
], JobCategory.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)({ type: 'varchar', nullable: false }),
    __metadata("design:type", String)
], JobCategory.prototype, "name", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => employer_profile_entity_1.EmployerProfile, { nullable: true, onDelete: 'CASCADE' }),
    (0, typeorm_1.JoinColumn)({ name: 'employer_id' }),
    __metadata("design:type", Object)
], JobCategory.prototype, "employer", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'is_active', type: 'boolean', default: true }),
    __metadata("design:type", Boolean)
], JobCategory.prototype, "isActive", void 0);
__decorate([
    (0, typeorm_1.Column)({
        name: 'created_at',
        type: 'timestamp',
        default: () => 'CURRENT_TIMESTAMP',
    }),
    __metadata("design:type", Date)
], JobCategory.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => job_entity_1.Job, (job) => job.category),
    __metadata("design:type", Array)
], JobCategory.prototype, "jobs", void 0);
exports.JobCategory = JobCategory = __decorate([
    (0, typeorm_1.Entity)('m_job_categories')
], JobCategory);
//# sourceMappingURL=job-category.entity.js.map